...
async function getJoke(event) {
    ...
    const URL = `${SERVER_URL}joke?about= + ${document.getElementById('traveltext').value}`
...
async function getJokeWithRateLimit(event) {
    ...
    const URL = `${SERVER_URL}jokelimited?traveltext= + ${document.getElementById('about2').value}`
